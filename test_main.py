...
def test_one_epoch(extractor, flow_model, val_dl, file_list, fpr):
    ...
    return np.mean(anomaly_score_list)
        ...
        for _id in id_list:
            ...
            if mode:
                ...
                for epoch in range(1, epochs+1):                  
                    ...
                    if epoch % 5 == 0:    
                        print("----------------- Evaluating -------------------")
                        val_anomaly_score = test_one_epoch(extractor=extractor, flow_model=flow_model, val_dl=val_dl, file_list=val_list, fpr=param['max_fpr'])
                            
                        com.logger.info("Anomaly score of {machine} {_id}: {anomaly_score}".format(machine=machine, _id=_id, anomaly_score=val_anomaly_score))
                        ...
                        if val_anomaly_score < anomaly_score:
